...
| `l`           | `nol`               | Seek forward 10s |
| `ArrowUp`     | `noarrowup`         | Turn volume up |
| `ArrowDown`   | `noarrowdown`       | Turn volume down |
| `<`           | `no<` or `no&lt;`   | Decrease playback rate |
| `>`           | `no>` or `no&gt;`   | Increase playback rate |
| `Shift + /`   | `noshift+/`         | Show keyboard shortcuts |

The keyboard shortcuts dialog opened with `Shift + /` only lists the shortcuts that are currently enabled, so turning one off also removes it from that dialog.
