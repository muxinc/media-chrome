...
<ComponentSandpack
  ...
  files={{
    "/console.js": `const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    if (mutation.type === "attributes" && mutation.attributeName.includes('breakpoint')) {
      console.info('media-controller breakpoint set to ' + mutation.attributeName);
    }
  });
});

observer.observe(document.querySelector('media-controller'), {
  attributes: true
});
    `
  }}
