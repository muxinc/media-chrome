...
You can either use the `<media-poster-component>` as a static image on its own, or, add the `slot="poster"` attribute to associate it with the underlying video element for click-to-play.
