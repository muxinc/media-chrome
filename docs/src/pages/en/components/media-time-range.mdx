...
Adding the `<track default label="thumbnails" kind="metadata" src="">` 
...
The `<media-time-range>` will be updated with **media state** attributes any time 
...
Discover all the available [media state attributes](#media-ui-attributes).
