...
A custom media element is not required to support the complete `HTMLMediaElement` API.
Each Media Chrome component requires different API's, for example to get the play
button to work properly the media element requirements might be:
...
The `HTMLVideoElement` API will be automatically added to any child element that
...
[hls-video-element](https://github.com/muxinc/hls-video-element) is a good example
