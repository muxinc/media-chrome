...
describe("<media-play-button/>", () => {
  ...
  describe('paused', () => {
    ...
    beforeEach(async () => {
      el = await fixture(`<media-play-button ${MediaUIAttributes.MEDIA_PAUSED}></media-play-button>`);
    });
  ...
  describe.skip("custom slots", () => {
    it("should support custom play content", async () => {
      ...
      const el = await fixture(`<media-play-button><span id="${idStr}" slot="${slotStr}">Play</span></media-play-button>`);
      const slottedEl = el.shadowRoot.querySelector(`slot[name="${slotStr}"] > *`);
