...
const MediaUIPropsEntries: Entry<MediaUIProps>[] = Object.entries(
  MediaUIProps
) as any as Entry<MediaUIProps>[];

export const MediaUIAttributes: Record<
  MediaUIPropsKey,
  Lowercase<MediaUIProps>
> = MediaUIPropsEntries.reduce(
  (dictObj, [key, propName]: Entry<MediaUIProps>) => {
    dictObj[key] = `${(propName as string).toLowerCase()}`;
    return dictObj;
  },
  {} as Record<MediaUIPropsKey, Lowercase<MediaUIProps>>
);

export const MediaStateChangeEvents =
  /** @type {{ [k in keyof MediaUIProps | 'USER_INACTIVE' | 'BREAKPOINTS_CHANGE' | 'BREAKPOINTS_COMPUTED']: string }} */ MediaUIPropsEntries.reduce(
...
export const AttributeToStateChangeEventMap = Object.entries(
  ...
  (mapObj, [key, attrName]) => {
    ...
    if (evtType) {
      mapObj[attrName as string] = evtType;
    }
    return mapObj;
  },
